...
##
func _update_direction_on_collision_on_wall(direction:Vector2, normal:Vector2)-> Vector2:
	var collinear_factor = 2*direction.dot(normal)
	direction -= collinear_factor*normal
	return direction


func _update_direction_on_collision_with_angle_offset(direction:Vector2, collision_info:KinematicCollision2D, angle_offset:float) -> Vector2:
#this only workd for the palette (or when the normal is vertical)
...
func _update_direction_on_collision(direction:Vector2, collision_info:KinematicCollision2D) -> Vector2:
	...
	if (collider.has_method("bounce_angle_offset")):
		...
		return _update_direction_on_collision_on_wall(direction, collision_info.normal)
